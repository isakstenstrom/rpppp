...
worker_do_tx(void *arg)
{
    ...
    while (!fdata->done)
    {
        ...
        if (cq_id >= lst_qid)
        {
            ...
            worker_fwd_event(&ev, RTE_SCHED_TYPE_ATOMIC);
            ev.queue_id = (cq_id == lst_qid) ? cdata.next_qid[ev.queue_id] : ev.queue_id;
        }
        else
        {
    ...
    while (!fdata->done)
    {
        ...
        for (i = 0; i < nb_rx; i++)
        {
            ...
            if (cq_id >= lst_qid)
            {
                ...
                ev[i].queue_id = (cq_id == lst_qid) ? cdata.next_qid[ev[i].queue_id] : ev[i].queue_id;
...
setup_eventdev_worker_tx_enq(struct worker_data *worker_data)
{
    ...
    for (i = 0; i < nb_queues; i++)
    {
        ...
        else
        {
            ...
            slot = i % nb_slots;
            wkr_q_conf.schedule_type = slot == cdata.num_stages ? RTE_SCHED_TYPE_ATOMIC : cdata.queue_type;
        }
    ...
    for (i = 0; i < cdata.num_workers; i++)
    {
        ...
        if (rte_event_port_link(dev_id, i, NULL, NULL, 0) != nb_queues)
        {
...
init_adapters(uint16_t nb_ports)
{
    ...
    for (i = 0; i < nb_ports; i++)
    {
        ...
        queue_conf.ev.queue_id = cdata.rx_stride ? (i * cdata.rx_stride)
                                                 : (uint8_t)cdata.qid[0];
        ...
        if (!(cap & RTE_EVENT_ETH_RX_ADAPTER_CAP_INTERNAL_PORT))
        {
            ...
            adptr_services->rx_adpt_arr = rte_realloc(
                ...
                adptr_services->nb_rx_adptrs *
                    sizeof(uint32_t),
                0);
            adptr_services->rx_adpt_arr[adptr_services->nb_rx_adptrs - 1] =
                service_id;
        }
