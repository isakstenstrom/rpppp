 ...
 * Copyright(c) 2023 Josef Olsson & Isak Stenström
 */
...
parse_app_args(int argc, char **argv)
{
    ...
    for (;;)
    {
        c = getopt_long(argc, argv, "r:t:e:c:w:n:f:s:m:l:paoPqDW:",
        ...
        case 'c':
            ...
            break;
        case 'W':
            cdata.org_worker_cycles = (unsigned int)atoi(optarg);
            break;
        case 'P':
        ...
        case 'q':
            ...
            break;
        case 'l':
            cdata.measure_latency = (unsigned int)atoi(optarg);
            break;
        case 'D':
            cdata.dump_dev = 1;
            break;
...
do_capability_setup(uint8_t eventdev_id)
{
    ...
    burst = eventdev_info.event_dev_cap & RTE_EVENT_DEV_CAP_BURST_MODE ? 1 : 0;
...
int main(int argc, char **argv)
{
    ...
    cdata.worker_cycles = eventdev_burn_tsc_to_loops(cdata.org_worker_cycles);
    ...
    uint64_t timer_stop = rte_get_timer_cycles();
                         ...
                         (uint64_t)-ENOTSUP))
    {
        ...
        double run_length = ((timer_stop - timer_start) * 1.0) / rte_get_tsc_hz();
        uint64_t done_work = tot_pkts * cdata.org_worker_cycles * cdata.num_stages;
        // +2 to account for the scheduler and rx/tx
        uint64_t ideal_work = (cdata.num_workers + 2) * run_length * rte_get_tsc_hz();

        printf("\n\n");

        if (cdata.measure_latency == 0)
        {
            printf("# AVG\n");
            {
                printf("%d\t%.4f\t%" PRIu64 "\t%" PRIu64 "\n", cdata.num_workers, (tot_pkts / run_length) / 1e6, done_work, ideal_work);
            }
        }

        if (cdata.measure_latency == 1)
        {
            printf("# TL\n");

            uint64_t tl_hist[MAX_LATENCY];
            for (unsigned i = 0; i < MAX_LATENCY; i++)
            {
                tl_hist[i] = 0;
            }
            eventdev_worker_tl_hist(worker_data, cdata.num_workers, tl_hist, MAX_LATENCY);
            uint64_t hist_length = hist_len(tl_hist, MAX_LATENCY);
            print_hist(tl_hist, false, hist_length);
        }

        if (cdata.measure_latency == 2)
        {
            for (uint32_t stage = 0; stage < cdata.num_stages; stage++)
            {
                printf("# TSL-%d\n", stage);

                uint64_t tsl_hist[MAX_LATENCY];
                for (unsigned i = 0; i < MAX_LATENCY; i++)
                {
                    tsl_hist[i] = 0;
                }
                eventdev_worker_tsl_hist(worker_data, cdata.num_workers, tsl_hist, MAX_LATENCY, stage);
                uint64_t hist_length = hist_len(tsl_hist, MAX_LATENCY);
                print_hist(tsl_hist, false, hist_length);
            }
        }
    }

    // for (uint32_t i = 0; i < cdata.num_workers; i++)
    // {
    //     free(worker_data[i].tl);
    //     free(worker_data[i].tsl);
    // }
