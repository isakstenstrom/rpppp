...
worker_generic(void *arg)
{
    ...
    {
        data->tl = rte_malloc(NULL, sizeof(uint64_t) * MAX_LATENCY, 0);
        for (int i = 0; i < MAX_LATENCY; i++)
        {
            data->tl[i] = 0;
        }
    }
    else if (cdata.measure_latency == 2)
    {
        for (uint32_t stage = 0; stage < cdata.num_stages; stage++)
        {
            data->tsl[stage] = rte_malloc(NULL, sizeof(uint64_t) * MAX_LATENCY, 0);
            for (int i = 0; i < MAX_LATENCY; i++)
            {
                data->tsl[stage][i] = 0;
            }
    ...
    while (!fdata->done)
    {
        ...
        {
            ev.flow_id = ev.mbuf->hash.rss % cdata.num_fids;
        }
        ...
        if (cdata.measure_latency == 2)
        {
            ...
            uint64_t tsl_us = eventdev_tsc_to_s((now - ev.mbuf->ol_flags) * 1000000);
            if (tsl_us >= MAX_LATENCY)
            {
                rte_panic("OH NO SWITCHING LATENCY TOO BIG :%lu\n", tsl_us);
            }
            else
            {
                data->tsl[ev.queue_id][tsl_us]++;
            }
        }
        ...
        eventdev_burn(cdata.worker_cycles);

        // Measure total latency
        if (cdata.measure_latency == 1 && ev.queue_id == cdata.num_stages)
        {
            uint64_t now = rte_get_timer_cycles();
            uint64_t tl_us = eventdev_tsc_to_s((now - ev.mbuf->ol_flags) * 1000000);
            if (tl_us >= MAX_LATENCY)
            {
                rte_panic("OH NO TOT LATENCY TOO BIG :%lu\n", tl_us);
            }
            else
            {
                data->tl[tl_us]++;
        ...
        // Set the ts latency for next stage
        if (cdata.measure_latency == 2)
        {
            ev.mbuf->ol_flags = rte_get_timer_cycles();
        }
...
setup_eventdev_generic(struct worker_data *worker_data)
{
    ...
    wkr_p_conf.event_port_cfg = disable_implicit_release ? RTE_EVENT_PORT_CFG_DISABLE_IMPL_REL : 0;
